...
//Interrupt call when a char is received
__attribute__((__interrupt__)) static void receive_interrupt(void)
{
	instruction = (char)AVR32_USART1.RHR.rxchr; // Read the char
	flag = AVR32_USART1.CSR.rxrdy; // Read/Clear the "receiver ready" flag
}
...
void USART_init(volatile avr32_usart_t *usart)
{
	...
	//Setup the receiver interrupt
	INTC_register_interrupt(&receive_interrupt,AVR32_USART1_IRQ ,AVR32_INTC_INT0);
	usart->IER.rxrdy = 1; // Enable interrupts on receiver
